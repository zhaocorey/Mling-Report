#!/usr/bin/env python3
"""M-DaQ Poster v6 for SIGIR 2026.
Fixes from v5:
  1. Larger fonts throughout (28-48pt)
  2. Fix all overlaps with strict zone enforcement
  3. Reduce whitespace between sections
  4. Better title layout (no wrapping)
"""
import os
...
logo_x = W - 4.5
logos = [
    ("huawei_logo.png", 0.2, 0.55),
    (SIGIR_LOGO, 0.85, 0.9),
    ("ustc_logo.png", 1.85, 0.55),
]
...
# ── Title (single line, large) ──
title_w = W - 5.5
tb(0.7, 0.25, title_w, 0.8,
   "M-DaQ: Retrieving Samples with Multilingual Diversity and Quality for Instruction Fine-Tuning Datasets",
   sz=40, bold=True, color=C_DARK, al=PP_ALIGN.LEFT)

# Authors (compact)
tb(0.7, 1.1, title_w, 0.35,
...
# Strict zones: 18% + 22% + 60%
# ═══════════════════════════════════════
Z1_END = YS + AVAIL * 0.18   # ~10.8"
Z2_END = YS + AVAIL * 0.40   # ~20.4"
Z3_END = FY                   # 46.45"
...
for cid, ch, sol in [
    ...
    multi(C1+0.5, y, CW-0.5, 0.6, [
        (ch, 21, False, C_BLACK, PP_ALIGN.LEFT, 1),
        (sol, 20, True, C_ACCENT, PP_ALIGN.LEFT, 2),
    ])
    y += 0.65

# Force to zone 2 start
y = max(y, Z1_END + 0.2)
...
# Force to zone 3 start
y = max(y, Z2_END + 0.2)

# Pipeline figure (zone 3, fills rest)
...
# Strict zones: 30% + 20% + 20% + 15% + 15%
# ═══════════════════════════════════════
Z2A_END = YS + AVAIL * 0.30   # ~16.0"
Z2B_END = YS + AVAIL * 0.50   # ~24.7"
Z2C_END = YS + AVAIL * 0.70   # ~33.4"
Z2D_END = YS + AVAIL * 0.85   # ~40.0"
Z2E_END = FY                   # 46.45"

y = YS

# 3. KEY RESULTS (zone A)
...
# Force to zone B start
y = max(y, Z2B_END + 0.2)

# 4. HUMAN EVALUATION (zone B)
...
# Force to zone C start
y = max(y + rh*nr + 0.3, Z2C_END + 0.2)
...
multi(C2+0.3, y+0.2, CW-0.6, ch-0.3, [
    ("📎  Code & Resources", 24, True, C_DARK, PP_ALIGN.LEFT, 8),
    ("     github.com/zhaocorey/M-DaQ", 20, False, C_BLACK, PP_ALIGN.LEFT, 10),
    ("  Paper", 24, True, C_DARK, PP_ALIGN.LEFT, 8),
    ("     DOI: 10.1145/3805712.3809946   |   arXiv: 2509.15549", 20, False, C_BLACK, PP_ALIGN.LEFT, 10),
    ("📧  Contact", 24, True, C_DARK, PP_ALIGN.LEFT, 8),
...
# ── Save ──
out = os.path.join(OUTPUT_DIR, "M-DaQ_poster_v6.pptx")
prs.save(out)
print(f"\n✅ PPTX: {out}")
