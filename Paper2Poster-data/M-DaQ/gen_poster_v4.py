#!/usr/bin/env python3
"""M-DaQ Poster v4 for SIGIR 2026.
A0 Portrait, 2-column. Light header. SIGIR logo.
"""
import os
...
def hdr(x, y, text, num=None):
    ...
    tb(x, y, CW, 0.55, label, sz=32, bold=True, color=C_DARK)
    line = sl.shapes.add_shape(1, Inches(x), Inches(y+0.55), Inches(CW), Inches(0.05))
...
ly = 0.3; lh = 1.0
for lp, lx, lh2 in [
    ("huawei_logo.png", 1.2, lh*0.7),
    ("sigir_logo.png", W/2 - 3.5, lh),
    ("ustc_logo.png", W - 5.5, lh*0.7),
]:
    p = os.path.join(LOGO_DIR, lp)
    if os.path.exists(p): img(p, lx, ly, height=lh2)

# Title
tb(0.8, 1.4, W-1.6, 1.2,
   "M-DaQ: Retrieving Samples with Multilingual Diversity\nand Quality for Instruction Fine-Tuning Datasets",
   sz=48, bold=True, color=C_DARK, al=PP_ALIGN.CENTER)
# Authors
tb(0.8, 2.55, W-1.6, 0.45,
   ...
   sz=20, color=C_BLACK, al=PP_ALIGN.CENTER)
# Affiliations
tb(0.8, 2.95, W-1.6, 0.4,
   "¹ Huawei Technologies Ltd.  ² USTC  |  SIGIR 2026 · Melbourne · July 20–24",
   sz=19, color=C_GRAY, al=PP_ALIGN.CENTER)
...
tb(C1, y, CW, 0.4, "Three Challenges Addressed:", sz=24, bold=True, color=C_DARK)
y += 0.55
for cid, ch, sol in [
    ...
    badge = sl.shapes.add_shape(1, Inches(C1), Inches(y), Inches(0.5), Inches(0.38))
    badge.fill.solid(); badge.fill.fore_color.rgb = C_ACCENT; badge.line.fill.background()
    tb(C1+0.03, y+0.02, 0.45, 0.35, cid, sz=17, bold=True, color=C_WHITE, al=PP_ALIGN.CENTER)
    multi(C1+0.65, y, CW-0.65, 0.85, [
        (ch, 22, False, C_BLACK, PP_ALIGN.LEFT, 2),
        (sol, 21, True, C_ACCENT, PP_ALIGN.LEFT, 4),
    ])
    y += 0.95
y += 0.3
...
tb(C1, y, CW, 0.4, "Stage 1: Quality Scoring Model (QSM)", sz=26, bold=True, color=C_ACCENT)
y += 0.5
blt(C1, y, CW, 3.0, [
...
y += 3.5
tb(C1, y, CW, 0.4, "Stage 2: Diversity-Aware Selection (DAS)", sz=26, bold=True, color=C_ACCENT)
y += 0.5
blt(C1, y, CW, 4.5, [
...
if pipe:
    ...
    tb(C1, y+fh+0.05, CW, 0.4,
       "Figure: M-DaQ two-stage pipeline (QSM + DAS)",
       sz=18, color=C_GRAY, al=PP_ALIGN.CENTER)

# ═══════════════════════════════════════
# COLUMN 2: Results → HumanEval → SAH → Conclusion → Contact
# Tight sequential layout, no wasted zones
# ═══════════════════════════════════════
y = YS
...
# Stats card
card = sl.shapes.add_shape(1, Inches(C2), Inches(y), Inches(CW), Inches(1.5))
card.fill.solid(); card.fill.fore_color.rgb = C_BG; card.line.fill.background()
multi(C2+0.3, y+0.1, CW-0.6, 1.3, [
    ("LLM-as-Judge: Avg Win Rate (M-DaQ vs Vanilla)", 24, True, C_DARK, PP_ALIGN.CENTER, 12),
    ("Alpaca-Eval: 60.2%  ·  MT-Bench R1: 62.6%  ·  MT-Bench R2: 62.9%",
     24, True, C_ACCENT, PP_ALIGN.CENTER, 4),
])
y += 1.8
...
tb(C2, y, CW, 0.4, "6 Languages · 900 Samples · 58 Person-Hours", sz=22, bold=True, color=C_DARK)
y += 0.5
...
for ci, h in enumerate(rows[0]):
    ...
    for p in cell.text_frame.paragraphs:
        ...
        for r in p.runs: r.font.size=Pt(19); r.font.bold=True; r.font.name="Arial"; sc(r, C_WHITE)
...
# Cultural example (compact)
cor = figs.get("fig_p4_0")
if cor and y + 3 < FY - 12:
    tb(C2, y, CW, 0.4, "Cultural Localization:", sz=24, bold=True, color=C_DARK)
    y += 0.5
    aspect = cor["h"] * CW / cor["w"]
    fh = min(3.5, aspect); fh = max(fh, 2.0)
    img(cor["path"], C2, y, width=CW)
    tb(C2, y+fh+0.05, CW, 0.5,
       "French query about Corsica → M-DaQ references Piana, figatelli", sz=17, color=C_GRAY, al=PP_ALIGN.CENTER)
    y += fh + 0.6
...
card.fill.solid(); card.fill.fore_color.rgb = C_BG; card.line.fill.background()
multi(C2+0.4, y+0.25, CW-0.8, ch-0.4, [
    ("📎  Code & Resources", 24, True, C_DARK, PP_ALIGN.LEFT, 10),
    ("     github.com/zhaocorey/M-DaQ", 20, False, C_BLACK, PP_ALIGN.LEFT, 12),
    ("📄  Paper", 24, True, C_DARK, PP_ALIGN.LEFT, 10),
    ("     DOI: 10.1145/3805712.3809946   |   arXiv: 2509.15549", 20, False, C_BLACK, PP_ALIGN.LEFT, 12),
    ("📧  Contact", 24, True, C_DARK, PP_ALIGN.LEFT, 10),
    ("     zhaochunguang6@huawei.com   |   +86-15501192353", 20, False, C_BLACK, PP_ALIGN.LEFT, 4),
])

# ── Footer ──
tb(0, FY, W, 0.4, "SIGIR 2026  ·  Melbourne, VIC, Australia  ·  July 20–24, 2026",
   sz=18, color=C_GRAY, al=PP_ALIGN.CENTER)
...
# ── Save ──
out = os.path.join(OUTPUT_DIR, "M-DaQ_poster_v4.pptx")
prs.save(out)
print(f"\n✅ PPTX: {out}")
