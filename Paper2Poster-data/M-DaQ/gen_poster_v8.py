#!/usr/bin/env python3
"""M-DaQ Poster v8 for SIGIR 2026.
Final fixes from v7:
  1. Reduce SAH figure height to prevent overlap with Conclusion
  2. Add explicit spacing between sections in column 2
  3. Reduce pipeline figure height in column 1
"""
import os
...
# Pipeline figure (REDUCED height to 10" max)
pipe = figs.get("fig_p1_1")
if pipe:
    remain = FY - y - 0.5
    aspect = pipe["h"] * CW / pipe["w"]
    # Cap at 10" height to avoid overflow
    fh = min(remain, aspect, 10.0)
    fh = max(fh, 5.0)
...
# 5. SAH (REDUCED figure height to 4" max)
...
if sah:
    ...
    fh = min(4.0, aspect)  # REDUCED from 5.0 to 4.0
    fh = max(fh, 2.5)
...
# ── Save ─
out = os.path.join(OUTPUT_DIR, "M-DaQ_poster_v8.pptx")
prs.save(out)
print(f"\n✅ PPTX: {out}")
