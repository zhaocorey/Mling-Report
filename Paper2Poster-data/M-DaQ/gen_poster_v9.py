#!/usr/bin/env python3
"""M-DaQ Poster v9 for SIGIR 2026.
Final fixes from v8:
  1. Drastically reduce figure heights (pipeline: 7", SAH: 3.5")
  2. Add minimum spacing between all sections
...
# Pipeline figure (DRASITICALLY REDUCED to 7" max)
pipe = figs.get("fig_p1_1")
if pipe:
    remain = FY - y - 0.5
    aspect = pipe["h"] * CW / pipe["w"]
    # Cap at 7" height
    fh = min(remain, aspect, 7.0)
    fh = max(fh, 4.0)
...
# 5. SAH (DRASITICALLY REDUCED to 3.5" max)
...
if sah:
    ...
    fh = min(3.5, aspect)  # REDUCED from 4.0 to 3.5
    fh = max(fh, 2.0)
...
# ── Save ──
out = os.path.join(OUTPUT_DIR, "M-DaQ_poster_v9.pptx")
prs.save(out)
print(f"\n✅ PPTX: {out}")
